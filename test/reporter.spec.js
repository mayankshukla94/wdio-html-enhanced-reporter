...
describe('Custom HtmlReporter', function () {
  ...
  it('should generate report file onRunnerEnd', function () {
    ...
    reporter.onRunnerStart({
      specs: ['file://test/specs/example1.spec.js'],
    });
    reporter.onSuiteStart(suite);
    reporter.onTestStart(test);
    ...
    expect(reporter.specs).to.include('file://test/specs/example1.spec.js');
  });
  ...
  it('should capture screenshots for a test', function () {
    ...
    reporter.onRunnerStart({ specs: ['file:///dummy.spec.js'] });
    reporter.onSuiteStart(suite);
    reporter.onTestStart(test);

    process.emit('test:screenshot', 'screenshots/test-snap-1.png');
    process.emit('test:screenshot', 'screenshots/test-snap-2.png');
    ...
    expect(foundTest.screenshots[0].path).to.equal('screenshots/test-snap-1.png');
    expect(foundTest.screenshots[1].path).to.equal('screenshots/test-snap-2.png');
  });
});
